...
export class DocumentQueueService {
  ...
  async enqueueProcessing(data: DocumentProcessingJobData): Promise<void> {
    ...
    await this.queue.add(DOCUMENT_PROCESSING_JOB, data, { ...DOCUMENT_JOB_DEFAULT_OPTIONS, jobId: data.documentId });
    this.logger.debug(`Queued processing for document ${data.documentId}`);
  }
}
